...
from embedding import get_dataset_embeddings
...
def train(data_path, baseline_strategy=None, model_path=None):
    ...
    train_data, train_labels_ = corpus[
        "TRAINING"]  # the _ is the spacy convention for the string representation (rather than int/float)
...
def evaluate(model_path, data_path, split='TESTING', contextual=True, combination='REDUCE_MEAN', test_sentence=None):
    """Evaluate model performance on a dataset

    Args:
        model (Any): [description]
        data_path (str): path to dataset in JSON lines format - embeddings
    """
    ...
    if test_sentence:
        embmodel, tokenizer = get_model(contextual=contextual, combination=combination, use_spacy=False)
        if not contextual:
            combination = None

        corpus = get_dataset_embeddings(test_sentence, embmodel, tokenizer=tokenizer,
...
if __name__ == '__main__':
    ...
    args = p.parse_args()
    test_sentence = {"text": "From the AP comes this story :",
    ...
    if args.task == 'train':
        ...
        if args.model_path:
            evaluate(args.model_path, args.embeddings_path, test_sentence=test_sentence)
        else:
